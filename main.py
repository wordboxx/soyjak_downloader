...
if __name__ == '__main__':
    ...
    # If the request fails, log the error and exit
        ...
        logging.error(
            'Failed to get maximum soyjak ID, booru.soy may be down.')
        logging.info('Exiting...')
        exit(1)

    post_id = file_management.count_local_soyjaks(
        file_management.default_directory)

    # Download all soyjaks
    while post_id <= maximum_soyjaks:
        logging.info('Getting soyjak %s / %s...', post_id, maximum_soyjaks)
        try:
            src, variant = downloader.get_image_info(post_id)
            downloader.download_image(
                src, post_id, variant, file_management.default_directory)
        except TypeError:
